...
export function ProxyLogFilter({ proxies, selectedIds, onToggle, onSelectIds }: Props) {
  ...
  return (
    <div className="log-filters mb-3">
      <button type="button" className="log-collapse-toggle" onClick={() => setOpen((current) => !current)} aria-expanded={open}>
          ...
          {proxies.length === 0 ? (
            ...
            <>
              <Stack direction="horizontal" gap={2} className="flex-wrap align-items-center mb-2">
                ...
                />
                <Button variant="outline-secondary" size="sm" onClick={() => onSelectIds(proxies.map((proxy) => proxy.id))}>
                  All
                </Button>
                ...
                {query && (
                  <Button
                    ...
                    onClick={() => onSelectIds([...new Set([...selectedIds, ...visible.map((proxy) => proxy.id)])])}
                  >
                    Add matches
                  </Button>
                )}
              ...
              <div className="proxy-log-list">
                ...
                {visible.length === 0 && <div className="mock-set-empty">No proxies match this search.</div>}
              </div>
            </>
          )}
        </div>
