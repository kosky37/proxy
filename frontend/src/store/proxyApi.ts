...
export const proxyApi = emptySplitApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    createMockSet: build.mutation<MockSetDto, { proxyId: string; body: MockSetDto }>({
      query: ({ proxyId, body }) => ({
        url: `/api/proxies/${proxyId}/mock-sets`,
        method: "POST",
        body,
      }),
    ...
    createIgnore: build.mutation<IgnoredPathDto, { proxyId: string; body: IgnoredPathDto }>({
      query: ({ proxyId, body }) => ({
        url: `/api/proxies/${proxyId}/ignores`,
        method: "POST",
        body,
      }),
      invalidatesTags: ["Ignores"],
    }),
    updateIgnore: build.mutation<
      IgnoredPathDto,
      { proxyId: string; name: string; body: IgnoredPathDto }
    >({
      query: ({ proxyId, name, body }) => ({
    ...
    sendManualRequest: build.mutation<
      LogDetailDto,
      { proxyId: string; body: ManualSendRequestDto }
    >({
