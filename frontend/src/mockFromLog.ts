...
export function mockFromLog(log: LogDetailDto): MockDto {
  ...
  const rawName = isSoap
    ? operation || localName(soapAction) || pathPart
    : `${log.method}-${pathPart}`;
  const responseType =
    contentTypeOnly(getHeader(responseHeaders, "Content-Type")) ??
    (isSoap ? "text/xml" : "application/json");
...
function findLocal(root: ParentNode, localName: string): Element | null {
  return (
    Array.from(root.querySelectorAll("*")).find((element) => element.localName === localName) ??
    null
  );
}
